...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Detect if user is asking about a specific tournament or event
    const bodyStr = JSON.stringify(req.body).toLowerCase();
    const isTournament = bodyStr.includes('march madness') || bodyStr.includes('tournament') || bodyStr.includes('playoff') || bodyStr.includes('first round') || bodyStr.includes('second round') || bodyStr.includes('sweet 16') || bodyStr.includes('elite 8') || bodyStr.includes('final four') || bodyStr.includes('world series') || bodyStr.includes('super bowl') || bodyStr.includes('stanley cup');

    const searchNote = config.web_search
      ? `You MUST use web search to look up current injuries, news, and today's schedule.${isTournament ? ' IMPORTANT: The user is asking about a specific tournament or event. You MUST use web search to find the EXACT games scheduled for this event TODAY. Do NOT use random games from the sport — only use games that are actually part of the tournament/event the user specified. For March Madness, search for "NCAA tournament games today" to find the actual bracket matchups.' : ''}`
    ...
    // Add web search only for pro/ultra or when no sports data available or tournament requests
    if (config.web_search || !hasSportsData || isTournament) {
