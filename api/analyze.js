...
export default async function handler(req, res) {
  ...
  try {
      ...
      if (msg.role === 'user' && hasSportsData) {
        ...
        const newContent = content.map(block => {
          if (block.type === 'text') {
            return {
              ...block,
              text: `HERE IS REAL-TIME SPORTS DATA for your analysis. Use this for games, odds, standings, and recent results. You may also use web search ONLY to look up injuries — do not search for odds or standings, it's already provided.\n\n${sportsDataBlock}\n---\n\n${block.text}`
            };
          }
          return block;
        });
    ...
    // Always include web search for injuries
    const requestBody = {
      ...req.body,
