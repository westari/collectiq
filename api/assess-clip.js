const RUBRICS = {
  ...
  shooting: {
    ...
    prompt: 'This is a shooting clip. Score the SHOOTER on shooting and shotForm. Look at: release point, elbow alignment, follow through, balance, arc, consistency, makes vs attempts. Score generously based on what you DO see — only return null if completely impossible to evaluate.',
  },
  finishing: {
  ...
  oneOnOne: {
    ...
    prompt: 'This is a 1-on-1 offensive possession. Score the BALL-HANDLER on ballHandling, creativity, finishing, and iq. Look at: ball handling under pressure, ability to create space, move variety, finishing creativity, decision making against the defender. Score generously based on what you DO see — only return null if completely impossible to evaluate.',
  },
};

const CALIBRATION = `
SCORING CALIBRATION - READ THIS CAREFULLY:
You are scoring competitive amateur and high school basketball players, not NBA players. Calibrate accordingly:
- 1-2 = Total beginner. Cannot perform the skill at all. Visible struggle on basics.
- 3-4 = Developing. Can sometimes execute but mechanics are inconsistent or weak. Below average for a competitive player.
- 5-6 = AVERAGE COMPETITIVE PLAYER. Solid fundamentals, repeatable mechanics, executes the skill reliably in normal situations. THIS IS THE BASELINE — most players you score should land here.
- 7-8 = Above average / advanced. Clean mechanics, consistent execution, would stand out on a competitive team. Varsity-level execution.
- 9-10 = Elite. College-recruit or higher. Reserve this for truly exceptional clips.

DEFAULT TO 5-7 RANGE. If a player demonstrates the skill cleanly even once, that is at least a 6. Do not punish players for what is not shown — only score what you actually see, and score it fairly. A clean repeatable shot form is a 7, not a 4. A confident layup with good touch is a 6, not a 3. Be honest but not harsh.
`;
...
export default async function handler(req, res) {
  ...
  const prompt = rubric.roleHint + ' ' + playerIdSection + rubric.prompt + CALIBRATION + ' Return ONLY valid JSON, no markdown, no backticks, no explanation. Format: {' + skillsList + ', "feedback": "1 sentence", "highlight": "best thing", "fix": "biggest fix"}';
